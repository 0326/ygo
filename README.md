...
支持 **简中 / 日文 / 英文** 三种语言卡图，自动回退策略确保总有可用卡图。

### 卡图变体

| 变体   | 用途               | 英文目录         | 简中目录           | 日文目录     |
| ------ | ------------------ | ---------------- | ------------------ | ------------ |
| `full` | 详情页主图（高清） | `cards/`         | `cards_cn/`        | `cards_jp/`  |
| `small`| 列表缩略图         | `cards_small/`   | `cards_cn_small/`  | `cards_jp/`  |
| `art`  | 制卡器裁剪图       | `cards_cropped/` | -（回退英文）      | -（回退英文）|

### 回退策略

- **简中**：cn → en（优先简中，缺失回退英文）
- **日文**：jp → en（优先日文，缺失回退英文）
- **英文**：en（无回退）

### 使用方式

前端通过 `?lang=cn|jp|en` 参数请求对应语言卡图：

```
/img/{key}?lang=cn       # 简中高清图
/img/{key}/s?lang=jp     # 日文缩略图
/img/{key}/art           # 制卡器裁剪图（语言无关，英文）
```

前端工具函数见 [`src/react-app/lib/cardImage.ts`](src/react-app/lib/cardImage.ts)，后端实现在 [`src/worker/lib/images.ts`](src/worker/lib/images.ts)。

### 缓存控制

卡图请求带 `_cv` 参数做缓存版本控制，修改 `CACHE_VERSION` 可强制刷新所有 CDN 缓存。
