...
export default function DeckBuilder() {
  ...
  const remember = useCallback(
    (c: CardSummary) => cache.current.set(c.id, c),
    [],
  );
  ...
  const pasteYdk = async () => {
    ...
    if (!text)
      text =
        window.prompt("粘贴 YDK 内容（#main / #extra / !side 卡密列表）") || "";
  ...
  const flash = (m: string) => {
    ...
    (flash as { t?: number }).t = window.setTimeout(() => setToast(""), 1800);
  };
  ...
  const add = (c: CardSummary, zone?: Zone) => {
    ...
    setDeck((d) => {
      ...
      if (z === "main" && isExtraCard(c)) {
        flash("融合/同调/超量/连接请放额外卡组");
        return d;
      }
      if (z === "extra" && !isExtraCard(c)) {
        flash("只有融合/同调/超量/连接能进额外卡组");
        return d;
      }
      if (z === "main" && d.main.length >= LIMITS.mainMax) {
        flash("主卡组已满 60");
        return d;
      }
      if (z === "extra" && d.extra.length >= LIMITS.extraMax) {
        flash("额外卡组已满 15");
        return d;
      }
      if (z === "side" && d.side.length >= LIMITS.sideMax) {
        flash("副卡组已满 15");
        return d;
      }
      return { ...d, [z]: [...d[z], c.id] };
  ...
  const clear = () => {
    if (confirm("清空当前卡组？")) setDeck(emptyDeck());
  };
  ...
  useEffect(() => {
    ...
    if (total) next.set("d", encodeDeck(deck));
    else next.delete("d");
  ...
  const saveDeckToAccount = async () => {
    if (!me) {
      nav(`/login?next=${encodeURIComponent("/deck" + location.search)}`);
      return;
    }
    const name = window.prompt(t("deck.saveName"), deckName || "我的卡组");
    if (!name || !name.trim()) return;
    try {
      const r = await saveMyDeck({
        id: savedId,
        name: name.trim(),
        deck_code: encodeDeck(deck),
        format,
      });
      setSavedId(r.id);
  ...
  const genImage = async () => {
    ...
    try {
      ...
      const canvas = await composeShareImage(items, {
        title: "我的卡组",
        subtitle: `主 ${deck.main.length} · 额外 ${deck.extra.length} · 副 ${deck.side.length}｜游戏王集卡社`,
        layout: "grid",
      });
      ...
      <div className="container">
        ...
        <div className="deck-bar">
          <div className="deck-counts">
            ...
            />
            <span
              className={`deck-valid ${v.ok ? "ok" : "bad"}`}
              title={v.errors.join("；")}
            >
          ...
          <div className="deck-actions">
            ...
            <button className="btn" onClick={() => void saveDeckToAccount()}>
              {t("deck.save")}
            </button>
            <button className="btn" onClick={() => fileRef.current?.click()}>
              {t("deck.importYdk")}
            </button>
            <button className="btn" onClick={pasteYdk}>
              {t("deck.pasteImport")}
            </button>
            <button className="btn" onClick={downloadYdk}>
              {t("deck.exportYdk")}
            </button>
            <button className="btn" onClick={genImage} disabled={busyImg}>
              {busyImg ? t("deck.generating") : t("deck.genImage")}
            </button>
                ...
                {results.map((c) => {
                  ...
                  return (
                    <div
                      ...
                      onKeyDown={(e) => {
                        if (e.key === "Enter")
                          add(c, e.shiftKey ? "side" : undefined);
                      }}
                      title={cname}
                    >
...
function DeckStatsPanel({
  ...
  const hand = 5;
  const probs = [3, 2, 1].map((n) => ({
    n,
    p: hyperAtLeast(mainSize, n, hand, 1),
  }));
  return (
          ...
          <div className="ds-attrs">
            {attrs.map(([a, n]) => (
              <span
                key={a}
                className="ds-attr"
                style={{ borderColor: ATTR_COLOR[a] || "#888" }}
              >
                {attrName(a, lang) || a} {n}
              </span>
            ))}
        ...
        <div className="ds-block">
          ...
          <div className="ds-curve">
            {stats.levelCurve.map((c) => (
              <div
                key={c.level}
                className="ds-bar-col"
                title={`${c.level} 级 · ${c.count} 张`}
              >
                <div
                  className="ds-bar"
...
function Count({
  ...
  return (
    <span className={`deck-count ${bad ? "bad" : ""}`}>
      {label} <b>{n}</b>
      {max != null && (
        <span className="lim">/{min != null ? `${min}-${max}` : max}</span>
      )}
    </span>
  );
}
...
function DeckZone({
  ...
  zone: Zone;
  deck: Deck;
  cache: Map<number, CardSummary>;
  onRemove: (z: Zone, id: number) => void;
  onAdd: (z: Zone, id: number) => void;
  format: BanFormat;
}) {
  const ids = deck[zone];
  const { lang, t } = useLang();
  ...
  return (
    <div
      ...
      onDragLeave={() => {
        ...
        if (!depth.current) setHover(false);
      }}
      onDrop={(e) => {
        e.preventDefault();
        depth.current = 0;
    ...
    >
      ...
      <div className="zone-grid">
        {ids.map((id, i) => {
          ...
          return (
            ...
            >
              ...
              {c?.attribute && (
                <span className="zone-attr">
                  <AttributeIcon attr={c.attribute} size={16} />
                </span>
              )}
              {c?.ban && (
                <span className="zone-ban">
                  <BanBadge ban={c.ban} format={format} dot />
                </span>
              )}
            </button>
        ...
        {!ids.length && (
          <div className="zone-empty muted">{t("deck.emptyZone")}</div>
        )}
      </div>
    </div>
  );
