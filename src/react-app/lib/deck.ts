...
import type { CardSummary, Frame, BanFormat, BanStatus } from "../../shared/types";
...
export function validate(deck: Deck, opts: ValidateOpts = {}): DeckValidation {
  ...
  if (cards) {
    const misplaced = (zone: number[], wantExtra: boolean) =>
      zone.some((id) => { const c = cards.get(id); return c && isExtraCard(c) !== wantExtra; });
    if (misplaced(deck.main, false)) errors.push("主卡组混入了额外卡组卡（融合/同调/超量/连接）");
  ...
  for (const [id, n] of seen) {
    ...
    if (n > max) {
      ...
      const tag = status === 0 ? "禁止卡" : status === 1 ? "限制(上限1)" : status === 2 ? "准限制(上限2)" : `上限 ${LIMITS.perCard}`;
...
export function fromYdk(text: string): Deck {
  ...
  for (const raw of text.split(/\r?\n/)) {
    ...
    if (line.startsWith("!side")) { zone = "side"; continue; }
...
export function deckStats(deck: Deck, cards: Map<number, CardSummary>): DeckStats {
  ...
  for (const id of deck.main) {
    ...
    if (c.card_type === "monster") {
      if (c.attribute) byAttribute[c.attribute] = (byAttribute[c.attribute] || 0) + 1;
  ...
  return { total: deck.main.length, byCardType, levelCurve, byAttribute, byRace };
}
...
export function hyperAtLeast(
  deckSize: number, successes: number, handSize: number, atLeast = 1,
): number {
  ...
  for (let k = 0; k <= maxFail; k++) {
    const ln = logChoose(successes, k) + logChoose(deckSize - successes, handSize - k) - denom;
